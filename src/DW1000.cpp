...
/*
const byte DW1000Class::MODE_LONGDATA_RANGE_LOWPOWER[] = {TRX_RATE_110KBPS, TX_PULSE_FREQ_16MHZ,
TX_PREAMBLE_LEN_2048}; const byte DW1000Class::MODE_SHORTDATA_FAST_LOWPOWER[] = {TRX_RATE_6800KBPS,
TX_PULSE_FREQ_16MHZ, TX_PREAMBLE_LEN_128}; const byte DW1000Class::MODE_LONGDATA_FAST_LOWPOWER[]  =
{TRX_RATE_6800KBPS, TX_PULSE_FREQ_16MHZ, TX_PREAMBLE_LEN_1024}; const byte
DW1000Class::MODE_SHORTDATA_FAST_ACCURACY[] = {TRX_RATE_6800KBPS, TX_PULSE_FREQ_64MHZ,
TX_PREAMBLE_LEN_128}; const byte DW1000Class::MODE_LONGDATA_FAST_ACCURACY[]  = {TRX_RATE_6800KBPS,
TX_PULSE_FREQ_64MHZ, TX_PREAMBLE_LEN_1024}; const byte DW1000Class::MODE_LONGDATA_RANGE_ACCURACY[] =
...
/*
const byte DW1000Class::BIAS_500_16[] = {198, 187, 179, 163, 143, 127, 109, 84, 59, 31, 0, 36, 65,
84, 97, 106, 110, 112}; const byte DW1000Class::BIAS_500_64[] = {110, 105, 100, 93, 82, 69, 51, 27,
0, 21, 35, 42, 49, 62, 71, 76, 81, 86}; const byte DW1000Class::BIAS_900_16[] = {137, 122, 105, 88,
69, 47, 25, 0, 21, 48, 79, 105, 127, 147, 160, 169, 178, 197}; const byte DW1000Class::BIAS_900_64[]
...
void DW1000Class::reset()
{
	...
	// dw1000 data sheet v2.08 §5.6.1 page 20, the RSTn pin should not be driven high but left
	// floating.
	GPIO_write(&_rst, LOW);
...
void DW1000Class::getPrintableExtendedUniqueIdentifier(char msgBuffer[])
{
	...
	sprintf(msgBuffer, "%02X:%02X:%02X:%02X:%02X:%02X:%02X:%02X", data[7], data[6], data[5],
...
void DW1000Class::getPrintableNetworkIdAndShortAddress(char msgBuffer[])
{
	...
	sprintf(msgBuffer, "PAN: %02X, Short Address: %02X", (uint16_t)((data[3] << 8) | data[2]),
...
void DW1000Class::getPrintableDeviceMode(char msgBuffer[])
{
	...
	sprintf(msgBuffer,
			"Data rate: %u kb/s, PRF: %u MHz, Preamble: %u symbols (code #%u), Channel: #%u", dr,
			prf, plen, pcode, ch);
}
...
void DW1000Class::setDefaults()
{
	...
	else if(_deviceMode == IDLE_MODE)
	{
		...
		/* old defaults with active frame filter - better set filter in every script where you
		really need it setFrameFilter(true);
...
void DW1000Class::correctTimestamp(DW1000Time& timestamp)
{
	...
	if(_channel == CHANNEL_4 || _channel == CHANNEL_7)
	{
		...
		if(_pulseFrequency == TX_PULSE_FREQ_16MHZ)
		{
			rangeBiasHigh = (rxPowerBaseHigh < BIAS_900_16_ZERO ? -BIAS_900_16[rxPowerBaseHigh] :
																	BIAS_900_16[rxPowerBaseHigh]);
			rangeBiasHigh <<= 1;
			rangeBiasLow = (rxPowerBaseLow < BIAS_900_16_ZERO ? -BIAS_900_16[rxPowerBaseLow] :
																  BIAS_900_16[rxPowerBaseLow]);
			rangeBiasLow <<= 1;
		}
		else if(_pulseFrequency == TX_PULSE_FREQ_64MHZ)
		{
			rangeBiasHigh = (rxPowerBaseHigh < BIAS_900_64_ZERO ? -BIAS_900_64[rxPowerBaseHigh] :
																	BIAS_900_64[rxPowerBaseHigh]);
			rangeBiasHigh <<= 1;
			rangeBiasLow = (rxPowerBaseLow < BIAS_900_64_ZERO ? -BIAS_900_64[rxPowerBaseLow] :
	...
	else
	{
		...
		if(_pulseFrequency == TX_PULSE_FREQ_16MHZ)
		{
			rangeBiasHigh = (rxPowerBaseHigh < BIAS_500_16_ZERO ? -BIAS_500_16[rxPowerBaseHigh] :
																	BIAS_500_16[rxPowerBaseHigh]);
			rangeBiasLow = (rxPowerBaseLow < BIAS_500_16_ZERO ? -BIAS_500_16[rxPowerBaseLow] :
																  BIAS_500_16[rxPowerBaseLow]);
		}
		else if(_pulseFrequency == TX_PULSE_FREQ_64MHZ)
		{
			rangeBiasHigh = (rxPowerBaseHigh < BIAS_500_64_ZERO ? -BIAS_500_64[rxPowerBaseHigh] :
																	BIAS_500_64[rxPowerBaseHigh]);
			rangeBiasLow = (rxPowerBaseLow < BIAS_500_64_ZERO ? -BIAS_500_64[rxPowerBaseLow] :
	...
	float rangeBias =
		rangeBiasLow + (rxPowerBase - rxPowerBaseLow) * (rangeBiasHigh - rangeBiasLow);
...
// Checks to see any of the three timeout bits in sysstatus are high (RXRFTO (Frame Wait timeout),
// RXPTO (Preamble timeout), RXSFDTO (Start frame delimiter(?) timeout).
...
float DW1000Class::getFirstPathPower()
{
	...
	float estFpPwr =
		10.0 * log10(((float)f1 * (float)f1 + (float)f2 * (float)f2 + (float)f3 * (float)f3) /
					 ((float)N * (float)N)) -
		A;
	if(estFpPwr <= -88)
	{
...
void DW1000Class::writeBytes(byte cmd, uint16_t offset, byte data[], uint16_t data_size)
{
	...
	SPI_transmitAndReceive(DW_1000_SPI_ID, spiTxBuf, spiRxBuf, headerLen + data_size,
						   SPI_TRANSFER_TIME_MS);
	delayMicroseconds(5);
	GPIO_write(&_ss, HIGH);
}
